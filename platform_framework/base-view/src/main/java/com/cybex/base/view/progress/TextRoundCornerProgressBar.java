...
public class TextRoundCornerProgressBar extends BaseRoundCornerProgressBar implements
    ...
    protected void initStyleable(Context context, AttributeSet attrs) {
        ...
        textProgressSize = (int) typedArray.getDimension(R.styleable.TextRoundCornerProgress_rcTextProgressSize,
                dp2px(DEFAULT_TEXT_SIZE));
        textProgressMargin = (int) typedArray.getDimension(R.styleable.TextRoundCornerProgress_rcTextProgressMargin,
...
//    }
    ...
    public void onGlobalLayout() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            tvProgress.getViewTreeObserver().removeOnGlobalLayoutListener(this);
        } else { tvProgress.getViewTreeObserver().removeGlobalOnLayoutListener(this); }
        drawTextProgressPosition();
    }
