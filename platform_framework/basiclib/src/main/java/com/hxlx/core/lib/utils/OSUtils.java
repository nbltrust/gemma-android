...
/**
 * 工具类: OS 系统相关
 * 简介 : 由于国内定制系统的泛滥, 不同定制系统的一些功能或实现方法会有所不同,
 * 如果需要做到足够好的适配工作, 需要
 * 对不同的定制系统做一些专门的适配.
 * API  : 获取 ROM 类型等
 */
        ...
        try {
            ...
            if (buildProperties.containsKey(KEY_MIUI_VERSION_NANE) || buildProperties.containsKey(
                ...
                if (buildProperties.containsKey(KEY_MIUI_VERSION_NANE)) {
                    ...
                    if (EmptyUtils.isNotEmpty(versionName) && versionName.matches("[Vv]\\d+")) { // V8
                        try {
                ...
                if (buildProperties.containsKey(KEY_MIUI_VERSION)) {
                    ...
                    if (EmptyUtils.isNotEmpty(versionStr) && versionStr.matches("[\\d.]+")) {
                ...
                if (buildProperties.containsKey(KEY_EMUI_VERSION)) {
                    ...
                    if (EmptyUtils.isNotEmpty(versionStr) && matcher.find()) {
                        try {
                ...
                if (buildProperties.containsKey(KEY_DISPLAY_ID)) {
                    ...
                    Matcher matcher = Pattern.compile("Flyme[^\\d]*([\\d.]+)[^\\d]*")
                ...
                if (buildProperties.containsKey(KEY_FUNTOUCHOS_OS_VERSION)) {
                    ...
                    if (EmptyUtils.isNotEmpty(versionStr) && versionStr.matches("[\\d.]+")) { // 3.0
                        try {
                            rom.setVersion(versionStr);
                ...
                if (EmptyUtils.isNotEmpty(displayId)) {
                ...
                if (EmptyUtils.isNotEmpty(baseOsVersion)) {
