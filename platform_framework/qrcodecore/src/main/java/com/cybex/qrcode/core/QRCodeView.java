...
public abstract class QRCodeView extends RelativeLayout implements Camera.PreviewCallback {
    ...
    public void onPreviewFrame(final byte[] data, final Camera camera) {
        if (QRCodeUtil.isDebug()) {
            QRCodeUtil.d("两次 onPreviewFrame 时间间隔：" + (System.currentTimeMillis() - sLastPreviewFrameTime));
            ...
            public void run() {
                try {
                    ...
                    for (PointF qrPoint : pointArr) {
                        transformedPoints[index] = transform(qrPoint.x, qrPoint.y, size.width, size.height,
                                isMirrorPreview, statusBarHeight, scanBoxAreaRect);
                        index++;
                    }
