...
import com.cybex.componentservice.api.data.response.LzyResponse;
import com.cybex.componentservice.api.data.response.SimpleResponse;
import com.cybex.componentservice.config.HttpConst;
...
public class JsonConvert<T> implements Converter<T> {
     ...
     * 主要作用是解析网络返回的 response 对象，
     * 生成onSuccess回调中需要的数据对象
     * 这里的解析工作不同的业务逻辑基本都不一样,需要根据不同业务实现
     */
    @Override
    ...
    private T parseParameterizedType(Response response, ParameterizedType type) {
        ...
        if (rawType != LzyResponse.class) {
            ...
            if (typeArgument == Void.class) {
                ...
                CustomData lzyResponse = GsonConvert.fromJson(jsonReader, type);
                response.close();
